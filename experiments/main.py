...
Instance_modified = namedtuple('Instance_modified',
                               'p t fv_leitner_diff fv_pimsleur_total fv_hlr_right fv_hlr_wrong fv_lr_time fv_optional_bias fv_optional_lexeme h a lang right wrong timestamp uid lexeme'.split())
...
def read_data(input_file, method, omit_bias=False, omit_lexemes=False, max_lines=None):
    ...
    for i, row in enumerate(reader):
        ...
        # instances_modified.append(
        #     Instance_modified(p, t, fv[0], fv[1], fv[2], fv[3], fv[4], fv[5], fv[6], h, (right + 2.) / (seen + 4.),
        #                       lang, right_this, wrong_this, timestamp, user_id, lexeme_string))
        if i % 1000000 == 0:
...
if __name__ == "__main__":
    ...
    # with open('data_postprocess_train.csv', 'w') as f:
    #     w = csv.writer(f)
    #     w.writerow(('p', 't', 'fv_leitner_diff', 'fv_pimsleur_total', 'fv_hlr_right', 'fv_hlr_wrong', 'fv_lr_time', 'fv_optional_bias', 'fv_optional_lexeme', 'h', 'a', 'lang', 'right', 'wrong',
    #                 'timestamp', 'uid', 'lexeme'))  # field header
    #     w.writerows([(data.p, data.t, data.fv_leitner_diff, data.fv_pimsleur_total, data.fv_hlr_right, data.fv_hlr_wrong, data.fv_lr_time, data.fv_optional_bias, data.fv_optional_lexeme, data.h, data.a, data.lang, data.right, data.wrong, data.timestamp, data.uid,
    #                   data.lexeme) for data in trainset])
